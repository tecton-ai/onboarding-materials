...
# MAGIC %md
# MAGIC # 📚 Feature Services, Historic & Batch Retrieval, and Online Retrieval from the Feature Store
# MAGIC 
...
# MAGIC When creating a new Feature Service, a completely new name can be defined; we can also create a variant of a Feature Service of the same name that already exists.  This will group the features together in the GUI.  The base tutorial takes you through creating a v2 of the `fraud_detection_feature_service` by adding the feature `user_has_great_credit` - this tutorial will take you through an example of culling the original service down to a subset as an example of some changes.  The changes will be the following:
# MAGIC 
# MAGIC Start with the original `fraud_detection_feature_service`, but
# MAGIC 1. Remove the on demand features `transaction_amount_is_high` and `transaction_amount_is_higher_than_average`.
# MAGIC 2. Currently the entire Feature View `user_transaction_amount_metrics` is brought in, which conists of means and sums across 4 windows apiece; 8 features.  Keep only the sum and mean for the 1 and 12 hour windows.
...
# MAGIC         user_transaction_amount_metrics[['amount_mean_1h_10m', 'amount_mean_12h_10m', 'amount_sum_1h_10m', 'amount_sum_12h_10m']],
...
display(training_events.drop("amount"))
...
training_data = fraud_detection_feature_service.get_historical_features(spine=training_events.drop("amount"), timestamp_key="timestamp", from_source=False).to_spark()
display(training_data)
